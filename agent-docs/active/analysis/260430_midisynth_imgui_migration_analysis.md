...
Following the avsamplerDOCKING pattern, this should be a **new project** (e.g., `MainImGui`):
...
│       ├── rack_imgui.lua      (rack module rendering)
│       ├── palette_imgui.lua   (palette browser rendering)
│       ├── keyboard_imgui.lua  (piano keyboard rendering)
...
`imguiRetainedPanel()` renders into an offscreen surface and blits it to ImGui. For the rack area with 9+ module shells, you get 9+ offscreen renders per frame. The avsamplerDOCKING approach mitigates this by grouping modules into fewer embed panels (e.g., one embed panel for the entire rack, with sub-layout done by ImGui child windows).
...
2. **Hybrid drag** — detect drag start from retained node mouse events, then show an ImGui floating window for the drag ghost. The avsamplerDOCKING shows context menus via ImGui but doesn't do drag-and-drop of modules.
