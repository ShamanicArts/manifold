...
void LooperProcessor::startRecording() {
    ...
    std::snprintf(json, sizeof(json),
        R"({"type":"record_start","mode":"%s"})",
        recordMode == RecordMode::FirstLoop ? "firstLoop" :
        recordMode == RecordMode::FreeMode ? "freeMode" :
        recordMode == RecordMode::Traditional ? "traditional" : "retrospective");
    pushEvent(json);
}

void LooperProcessor::startOverdub() {
...
void LooperProcessor::stopRecording() {
    ...
    switch (recordMode) {
        ...
        case RecordMode::Traditional: processTraditionalStop(); break;
        case RecordMode::Retrospective: break;
    }

    // Broadcast event
    char json[256];
    std::snprintf(json, sizeof(json),
        R"({"type":"record_stop","duration":%.4f})",
...
void LooperProcessor::commitRetrospective(float numBars) {
    ...
    commitRetrospectiveNow(numBars, activeLayerIndex, shouldOverdubLayer(activeLayerIndex));
}

void LooperProcessor::scheduleForwardCommit(float numBars) {
    if (numBars <= 0.0f) return;
...
void LooperProcessor::processFirstLoopStop() {
    ...
    if (result.valid) {
        ...
        layers[activeLayerIndex].copyFromCapture(captureBuffer, startOffset, samples, overdub);
        ++commitCount;

        // Broadcast events
        char json[256];
        std::snprintf(json, sizeof(json),
            R"({"type":"tempo","bpm":%.2f})", tempo);
        pushEvent(json);
...
void LooperProcessor::processFreeModeStop() {
    ...
    const bool overdub = shouldOverdubLayer(activeLayerIndex);
    layers[activeLayerIndex].copyFromCapture(captureBuffer, startOffset, quantizedLength, overdub);
    ++commitCount;
...
void LooperProcessor::commitRetrospectiveNow(float numBars, int layerIndex, bool overdub) {
    if (numBars <= 0.0f) return;
    if (layerIndex < 0 || layerIndex >= MAX_LAYERS) return;

    int samplesBack = static_cast<int>(numBars * getSamplesPerBar());
    if (samplesBack <= 0) return;
    ...
    if (overdub)
        layers[layerIndex].overdubFromCapture(captureBuffer, startOffset, samplesBack);
    else
...
void LooperProcessor::maybeFireForwardCommit() {
    ...
    const double armPlayTime = forwardCommitArmPlayTime.load(std::memory_order_relaxed);
    ...
    char json[256];
    std::snprintf(json, sizeof(json),
        R"({"type":"forward_fired","layer":%d,"bars":%.4f})",
        layer, bars);
    pushEvent(json);
}
...
bool LooperProcessor::postControlCommand(ControlCommand::Type type, int intParam, float floatParam) {
    ControlCommand cmd;
    cmd.type = type;
...
void LooperProcessor::updateAtomicState(const juce::AudioBuffer<float>& buffer) {
    auto& state = controlServer.getAtomicState();
    ...
    state.recordMode.store(static_cast<int>(recordMode), std::memory_order_relaxed);
    ...
    state.uptimeSeconds.store(playTime / currentSampleRate, std::memory_order_relaxed);
    ...
    for (int i = 0; i < MAX_LAYERS; ++i) {
        ...
        auto& layer = layers[i];
        ls.state.store(static_cast<int>(layer.getState()), std::memory_order_relaxed);
        ...
        float bars = (spb > 0 && layer.getLength() > 0) ? layer.getLength() / spb : 0.0f;
