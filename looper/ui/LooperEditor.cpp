...
namespace {
constexpr std::array<float, 9> kSegmentBars{
...
float steppedSpeed(float current, int direction) {
    constexpr std::array<float, 7> kSpeeds{0.25f, 0.5f, 0.75f, 1.0f, 1.5f, 2.0f, 4.0f};
...
float steppedLayerVolume(float current, int direction) {
    constexpr std::array<float, 9> kVolumes{0.0f, 0.25f, 0.5f, 0.75f, 1.0f, 1.25f, 1.5f, 1.75f, 2.0f};
...
void LooperEditor::paint(juce::Graphics& g) {
    juce::ColourGradient bg(
        juce::Colour(0xff161b26),
        0.0f,
        0.0f,
        juce::Colour(0xff0c1019),
        0.0f,
        (float)getHeight(),
        false);
...
void LooperEditor::buildCanvasUi() {
    ...
    titleNode->onDraw = [this](Canvas&, juce::Graphics& g) {
        ...
        g.drawText("LOOPER", b.removeFromLeft(230), juce::Justification::centredLeft);

        g.setFont(juce::Font("Avenir Next", 14.0f, juce::Font::plain));
        g.setColour(juce::Colour(0xff9ca3af));
        g.drawText("Capture plane + composable controls", b, juce::Justification::centredRight);
    };

    controlsNode = rootCanvas.addChild("controls");
    controlsNode->style = controlsNode->style.withBackground(juce::Colour(0xff141a24)).withCornerRadius(10.0f);

    auto makeControlButton = [this](Canvas* parent, const juce::String& name, std::function<void()> onClick) {
    ...
    recButton->onDraw = [this](Canvas& c, juce::Graphics& g) {
        ...
        drawPillButton(c, g, recording ? "REC*" : "REC", recording ? juce::Colour(0xffdc2626) : juce::Colour(0xff7f1d1d));
    };
    ...
    overdubButton->onDraw = [this](Canvas& c, juce::Graphics& g) {
        ...
        drawPillButton(c, g, active ? "OVERDUB*" : "OVERDUB", active ? juce::Colour(0xfff59e0b) : juce::Colour(0xff7c4a03));
    };
    ...
    modeButton->onDraw = [this](Canvas& c, juce::Graphics& g) {
        drawPillButton(c, g, recordModeText(processor.getRecordMode()), juce::Colour(0xff1f4a7a));
    };

    clearButton = makeControlButton(controlsNode, "clear", [this] {
        processor.postControlCommand(ControlCommand::Type::LayerClear, processor.getActiveLayerIndex());
    });
    ...
    tempoDownButton = makeControlButton(controlsNode, "tempo_down", [this] {
        processor.postControlCommand(ControlCommand::Type::SetTempo, 0, steppedTempo(processor.getTempo(), -1));
    });
    ...
    volumeDownButton = makeControlButton(controlsNode, "vol_down", [this] {
        processor.postControlCommand(ControlCommand::Type::SetMasterVolume, 0, steppedMasterVolume(processor.getMasterVolume(), -1));
    });
    ...
    captureNode->style = captureNode->style.withBackground(juce::Colour(0xff101723)).withCornerRadius(10.0f);
    captureNode->onDraw = [this](Canvas&, juce::Graphics& g) {
        ...
        if (forwardArmed) {
            caption << "  |  FORWARD ARMED " << juce::String(processor.getForwardCommitBars(), 3) << " bars";
        ...
        g.drawText("older", wave.removeFromLeft(52), juce::Justification::centredLeft);
        g.drawText("now", wave.removeFromRight(52), juce::Justification::centredRight);
    };
    ...
    for (int slot = 0; slot < (int)kSegmentBars.size(); ++slot) {
        ...
        const float startBars = (barsIndex > 0) ? kSegmentBars[(size_t)(barsIndex - 1)] : 0.0f;
        const juce::String label = strip.label;
        strip.node->onDraw = [this, startBars, endBars, label](Canvas& c, juce::Graphics& g) {
            const int rangeStart = (int)std::round(startBars * processor.getSamplesPerBar());
            const int rangeEnd = (int)std::round(endBars * processor.getSamplesPerBar());
            ...
            g.setColour(hasRange ? juce::Colour(0xff0f1b2d) : juce::Colour(0xff111827));
            g.fillRect(r);
            ...
            if (hasRange) {
                drawCaptureWindowWaveform(g,
                                          c.getLocalBounds().reduced(2),
                                          clippedStart,
                                          clippedEnd,
            ...
            g.setColour(hasRange ? juce::Colour(0xffcbd5e1) : juce::Colour(0xff6b7280));
            g.setFont(juce::Font("Avenir Next", 10.0f, juce::Font::bold));
            g.drawText(label, c.getLocalBounds().reduced(4, 0), juce::Justification::bottomLeft);
        };
    ...
    for (int i = (int)kSegmentBars.size() - 1; i >= 0; --i) {
        ...
        seg.node->onDraw = [this, bars, label](Canvas& c, juce::Graphics& g) {
            ...
            const bool armed = processor.isForwardCommitArmed() && std::abs(processor.getForwardCommitBars() - bars) < 0.001f;
            ...
            if (hovered || armed) {
                g.setColour(armed ? juce::Colour(0xffd9f99d) : juce::Colour(0xffbfdbfe));
                g.setFont(juce::Font("Avenir Next", 12.0f, juce::Font::bold));
                g.drawText(label + " bars", c.getLocalBounds().reduced(6, 0), juce::Justification::topRight);
            }
        };
    ...
    layersNode->style = layersNode->style.withBackground(juce::Colour(0xff0f1622)).withCornerRadius(10.0f);

    auto makeLayerActionButton = [](Canvas* row, const juce::String& name, std::function<void()> action) {
    ...
    for (int i = 0; i < LooperProcessor::MAX_LAYERS; ++i) {
        ...
        row->onDraw = [this, i](Canvas& c, juce::Graphics& g) {
            ...
            auto r = c.getLocalBounds().toFloat().reduced(1.0f);
            const auto base = active ? juce::Colour(0xff25405f) : juce::Colour(0xff1b2636);
            ...
            g.drawText("L" + juce::String(i), metaArea.removeFromLeft(28), juce::Justification::centredLeft);

            g.setColour(layerStateColour(layer.getState()));
            g.setFont(juce::Font("Avenir Next", 13.0f, juce::Font::plain));
            g.drawText(layerStateText(layer.getState(), layer.getLength()), metaArea.removeFromLeft(180), juce::Justification::centredLeft);

            g.setColour(juce::Colour(0xffcbd5e1));
            const juce::String details =
                "speed " + juce::String(layer.getSpeed(), 2) +
                (layer.isReversed() ? " | rev" : "") +
                " | vol " + juce::String(layer.getVolume(), 2);
            ...
            drawLayerWaveform(g, waveformArea.reduced(2), layer, active ? juce::Colour(0xff22d3ee) : juce::Colour(0xff38bdf8));
            ...
            if (layer.getLength() > 0 &&
                ...
                const int x = waveformArea.getX() + (int)std::round(pos * (float)waveformArea.getWidth());
                g.setColour(juce::Colour(0xffff4d4d));
                g.drawVerticalLine(x, (float)waveformArea.getY() + 1.0f, (float)waveformArea.getBottom() - 1.0f);
            }
        };

        layerRows[(size_t)i] = row;

        layerMuteButtons[(size_t)i] = makeLayerActionButton(row, "mute_" + juce::String(i), [this, i] {
            auto& layer = processor.getLayer(i);
            const bool currentlyMuted = layer.getState() == LooperLayer::State::Muted;
            processor.postControlCommand(ControlCommand::Type::LayerMute, i, currentlyMuted ? 0.0f : 1.0f);
        });
        layerMuteButtons[(size_t)i]->onDraw = [this, i](Canvas& c, juce::Graphics& g) {
            const bool on = processor.getLayer(i).getState() == LooperLayer::State::Muted;
            drawPillButton(c, g, "M", on ? juce::Colour(0xffef4444) : juce::Colour(0xff475569));
        };

        layerReverseButtons[(size_t)i] = makeLayerActionButton(row, "rev_" + juce::String(i), [this, i] {
            auto& layer = processor.getLayer(i);
            processor.postControlCommand(ControlCommand::Type::LayerReverse, i, layer.isReversed() ? 0.0f : 1.0f);
        });
        layerReverseButtons[(size_t)i]->onDraw = [this, i](Canvas& c, juce::Graphics& g) {
            const bool on = processor.getLayer(i).isReversed();
            drawPillButton(c, g, "R", on ? juce::Colour(0xff16a34a) : juce::Colour(0xff475569));
        };

        layerStopButtons[(size_t)i] = makeLayerActionButton(row, "stop_" + juce::String(i), [this, i] {
        ...
        layerClearButtons[(size_t)i] = makeLayerActionButton(row, "clear_" + juce::String(i), [this, i] {
        ...
        layerSpeedDownButtons[(size_t)i] = makeLayerActionButton(row, "speed_down_" + juce::String(i), [this, i] {
            auto& layer = processor.getLayer(i);
            processor.postControlCommand(ControlCommand::Type::LayerSpeed, i, steppedSpeed(layer.getSpeed(), -1));
        });
        layerSpeedDownButtons[(size_t)i]->onDraw = [](Canvas& c, juce::Graphics& g) {
            drawPillButton(c, g, "-", juce::Colour(0xff334155));
        };

        layerSpeedUpButtons[(size_t)i] = makeLayerActionButton(row, "speed_up_" + juce::String(i), [this, i] {
            auto& layer = processor.getLayer(i);
            processor.postControlCommand(ControlCommand::Type::LayerSpeed, i, steppedSpeed(layer.getSpeed(), +1));
        });
        ...
        layerVolumeDownButtons[(size_t)i] = makeLayerActionButton(row, "volume_down_" + juce::String(i), [this, i] {
            auto& layer = processor.getLayer(i);
            processor.postControlCommand(ControlCommand::Type::LayerVolume, i, steppedLayerVolume(layer.getVolume(), -1));
        });
        layerVolumeDownButtons[(size_t)i]->onDraw = [](Canvas& c, juce::Graphics& g) {
            drawPillButton(c, g, "V-", juce::Colour(0xff3f3f46));
        };

        layerVolumeUpButtons[(size_t)i] = makeLayerActionButton(row, "volume_up_" + juce::String(i), [this, i] {
            auto& layer = processor.getLayer(i);
            processor.postControlCommand(ControlCommand::Type::LayerVolume, i, steppedLayerVolume(layer.getVolume(), +1));
        });
    ...
    statusNode->style = statusNode->style.withBackground(juce::Colour(0xff0b1220)).withCornerRadius(8.0f);
    statusNode->onDraw = [this](Canvas&, juce::Graphics& g) {
        ...
        const auto left =
            "Tempo " + juce::String(processor.getTempo(), 2) +
            " BPM  |  1 bar " + juce::String(barSeconds, 3) + " s  |  target " +
            juce::String(processor.getTargetBPM(), 1) +
            "  |  master " + juce::String(processor.getMasterVolume(), 2) +
            (processor.isOverdubEnabled() ? "  |  overdub ON" : "  |  overdub OFF");
        g.drawText(left, b.removeFromLeft(statusNode->getWidth() - 180), juce::Justification::centredLeft);
...
void LooperEditor::layoutCanvasUi() {
    if (titleNode == nullptr || controlsNode == nullptr || captureNode == nullptr || layersNode == nullptr || statusNode == nullptr)
        return;
    ...
    const int buttonWidth = (ctl.getWidth() - kControlGap * (kControlCount - 1)) / kControlCount;
    ...
    for (int slot = 0; slot < (int)captureStrips.size(); ++slot) {
        ...
        if (node == nullptr)
            continue;
        node->setBounds(x0 + slot * slotWidth, captureArea.getY(), slotWidth, captureArea.getHeight());
    }
    ...
    if (captureNowIndicator != nullptr) {
        captureNowIndicator->setBounds(x0 + totalWidth - 2, captureArea.getY(), 2, captureArea.getHeight());
    }

    auto layerArea = layersNode->getLocalBounds().reduced(8);
    constexpr int rowGap = 8;
    const int rowHeight = (layerArea.getHeight() - rowGap * (LooperProcessor::MAX_LAYERS - 1)) / LooperProcessor::MAX_LAYERS;
...
void LooperEditor::refreshUiState() {
    rootCanvas.repaint();
}
...
juce::String LooperEditor::recordModeText(RecordMode mode) const {
    switch (mode) {
        case RecordMode::FirstLoop: return "First Loop";
        case RecordMode::FreeMode: return "Free Mode";
        case RecordMode::Traditional: return "Traditional";
        case RecordMode::Retrospective: return "Retrospective";
        default: return "Mode";
    }
}

juce::String LooperEditor::layerStateText(LooperLayer::State state, int lengthSamples) const {
    const float seconds = (float)lengthSamples / (float)juce::jmax(1.0, processor.getSampleRate());
    switch (state) {
        case LooperLayer::State::Empty: return "Empty";
        case LooperLayer::State::Playing: return "Playing " + juce::String(seconds, 2) + " s";
        case LooperLayer::State::Recording: return "Recording";
        case LooperLayer::State::Overdubbing: return "Overdubbing";
        case LooperLayer::State::Muted: return "Muted";
        case LooperLayer::State::Stopped: return "Stopped";
...
juce::Colour LooperEditor::layerStateColour(LooperLayer::State state) const {
    switch (state) {
        case LooperLayer::State::Empty: return juce::Colour(0xff64748b);
        case LooperLayer::State::Playing: return juce::Colour(0xff34d399);
        case LooperLayer::State::Recording: return juce::Colour(0xffef4444);
        case LooperLayer::State::Overdubbing: return juce::Colour(0xfff59e0b);
        case LooperLayer::State::Muted: return juce::Colour(0xff94a3b8);
        case LooperLayer::State::Stopped: return juce::Colour(0xfffde047);
...
void LooperEditor::drawLayerWaveform(juce::Graphics& g,
    ...
    const float rescale = highest > 0.0f ? juce::jlimit(1.0f, 8.0f, 1.0f / highest) : 1.0f;
...
void LooperEditor::drawCaptureWindowWaveform(juce::Graphics& g,
    ...
    for (int x = 0; x < width; ++x) {
        const float t = width > 1 ? (float)(width - 1 - x) / (float)(width - 1) : 0.0f;
    ...
    const float rescale = highest > 0.0f ? juce::jlimit(1.0f, 10.0f, 1.0f / highest) : 1.0f;
