...
public:
    ...
    void setAttack(float ms) { targetAttackMs_.store(juce::jlimit(0.01f, 500.0f, ms), std::memory_order_release); }
    void setRelease(float ms) { targetReleaseMs_.store(juce::jlimit(0.1f, 5000.0f, ms), std::memory_order_release); }
    void setSensitivity(float scale) { targetSensitivity_.store(juce::jlimit(0.01f, 16.0f, scale), std::memory_order_release); }
    void setHighpass(float hz) { targetHighpassHz_.store(juce::jlimit(5.0f, 4000.0f, hz), std::memory_order_release); }
    void setMode(int mode) { detectorMode_.store(juce::jlimit(0, 2, mode), std::memory_order_release); }
    ...
    int getMode() const { return detectorMode_.load(std::memory_order_acquire); }
...
private:
    ...
    std::atomic<int> detectorMode_{0};  // 0=peak, 1=RMS, 2=peak+RMS hybrid
