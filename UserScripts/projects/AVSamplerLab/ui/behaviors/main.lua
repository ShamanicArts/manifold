...
local function applyMappingTrack(ctx, track)
  local mapping = ctx.mappings[track]
  if not mapping.enabled then return end
  local src = clamp(poseSourceValue(ctx, track), 0, 1)
  if not mapping.invert then src = 1.0 - src end
...
local function applyMapping(ctx)
  local any = false
  for t=1,2 do if ctx.mappings[t].enabled then any = true; applyMappingTrack(ctx, t) end end
  if not any then setText(ctx.widgets.mappingStatus, "Mapping: disabled"); return end
  local m1 = ctx.mappings[1]
  local src1 = clamp(poseSourceValue(ctx, 1), 0, 1)
  if not m1.invert then src1 = 1.0 - src1 end
  local out1 = m1.min + src1 * (m1.max - m1.min)
  setText(ctx.widgets.mappingStatus, string.format("T1: %.3f→%.3f  T2: %s", src1, out1, ctx.mappings[2].enabled and "on" or "off"))
end
...
local function refreshShaderLists(ctx)
  ...
  setOptions(ctx.widgets.mapping1Source, poseNames)
  setOptions(ctx.widgets.mapping2Source, poseNames)
end
...
local function syncParamsFromHost(ctx)
  ...
  for t=1,2 do
    ...
    m.target = math.max(1, math.min(4, round(readParam(NS.."/mapping/"..t.."/target", m.target or 1))))
    m.min = readParam(NS.."/mapping/"..t.."/min", m.min or 0)
    m.max = readParam(NS.."/mapping/"..t.."/max", m.max or 1)
...
function M.init(ctx)
  ...
  ctx.captureRecording = false
  ctx.mappings = {
    { enabled=true, source=29, target=1, min=0.0, max=1.0, invert=false },
    { enabled=true, source=32, target=1, min=0.0, max=1.0, invert=false },
  }
  ctx.fxSlot = 1
  ...
  for t=1,2 do
    for _, id in ipairs({"mapping"..t.."Min", "mapping"..t.."Max"}) do
      ctx.widgets[id]._onChange = function(v)
        ...
        ctx.mappings[track][key] = tonumber(v) or 0
    ...
    ctx.widgets["mapping"..t.."Target"]._onSelect = function(idx)
      ctx.mappings[t].target = round(idx); writeParam(NS.."/mapping/"..t.."/target", ctx.mappings[t].target); saveState(ctx)
    end
