...
function buildPlugin(ctx)
  ...
  local function noteOff(note)
    local index = noteToVoice[round(clamp(note, 0, 127))]
  ...
  local regs = {
    ...
    { "/mapping/1/enabled", 0, 1, 1 }, { "/mapping/1/source", 1, 64, 29 }, { "/mapping/1/target", 1, 64, 1 }, { "/mapping/1/min", -8, 8, 0 }, { "/mapping/1/max", -8, 8, 1 }, { "/mapping/1/invert", 0, 1, 0 },
    { "/mapping/2/enabled", 0, 1, 1 }, { "/mapping/2/source", 1, 64, 32 }, { "/mapping/2/target", 1, 64, 1 }, { "/mapping/2/min", -8, 8, 0 }, { "/mapping/2/max", -8, 8, 1 }, { "/mapping/2/invert", 0, 1, 0 },
  }
  ...
  local function onParamChange(path, value)
    ...
    elseif path == NS .. "/play_trigger" then local n=round(value); if n~=counters.play then counters.play=n; if params.mode < 0.5 then triggerVoice(1, params.rootNote, 127) else triggerSlice(params.selectedSlice, 127) end end
    ...
    elseif path == NS .. "/midi_note_on_trigger" then local note, velocity, n = decodeMidiTrigger(value); if n ~= counters.noteOn then counters.noteOn = n; if params.mode < 0.5 then triggerVoice(allocateVoice(note), note, velocity) else local s=noteToSlice(note, params.rootNote); if s then triggerSlice(s, velocity) end end end
