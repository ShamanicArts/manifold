...
function M.create(deps)
  local Runtime = require(assert(deps.runtimeRequire, "voice_transform adapter needs runtimeRequire"))
  local ctx = deps.ctx
  local moduleId = tostring(deps.moduleId or "standalone_voice_fx_1")
  local specId = tostring(deps.specId or "voice_transform")
  ...
  local function resolveOutput(slot)
    local bundle = Runtime.resolveVoiceBundleSample(ctx, specId .. ".voice", sourceEndpoint, slot.index, clamp)
