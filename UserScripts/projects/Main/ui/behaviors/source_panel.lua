-- Legacy source panel behavior.
--
-- Despite the historical filename/component naming, this is not a focused
-- oscillator module. It owns the broader source section UI, including:
--   - Wave mode
--   - Sample mode/capture/loop editing
--   - Blend mode
--   - Shared source preview/graph state
--
-- Keep this behavior stable while extracting new rack modules out of it.
local SourcePanelBehavior = {}
...
local function getCachedAdditiveRecipe(ctx, waveType, pulseWidth, partialCountOverride)
  local width = math.max(0.01, math.min(0.99, tonumber(pulseWidth) or 0.5))
  local partialCount = math.max(1, math.min(32, math.floor((tonumber(partialCountOverride) or tonumber(ctx.additivePartials) or 8) + 0.5)))
...
local function getWaveAddPreviewVoiceConfig(ctx)
  local requestedUnison = math.max(1, math.min(8, math.floor((tonumber(ctx.unison) or 1) + 0.5)))
  local requestedDetune = math.max(0.0, math.min(100.0, tonumber(ctx.detune) or 0.0))

  local targetUnison = requestedUnison
  local targetDetune = requestedDetune
  if requestedUnison > 3 then
    local extra = (requestedUnison - 3) / math.max(1, 8 - 3)
    targetUnison = 3
    targetDetune = math.max(0.0, math.min(100.0, requestedDetune * (1.0 + extra * 1.15)))
  end

  return targetUnison, targetDetune
end

local function getCachedDetuneRatios(ctx, unisonOverride, detuneOverride)
  local unison = math.max(1, math.min(8, math.floor((tonumber(unisonOverride) or tonumber(ctx.unison) or 1) + 0.5)))
  local detune = math.max(0.0, math.min(100.0, tonumber(detuneOverride) or tonumber(ctx.detune) or 0.0))
...
local function additiveSuperSawPreview(ctx, phase, layerCountOverride)
  local detunes = { -18.0, -7.0, 0.0, 8.0, 19.0 }
  local gains = { 0.55, 0.82, 1.0, 0.79, 0.50 }
  local phases = { 0.17, 0.51, 0.0, 0.33, 0.74 }
  local layerCount = math.max(1, math.min(#detunes, math.floor((tonumber(layerCountOverride) or tonumber(ctx.additivePartials) or 8) + 0.5)))
  local sum = 0.0
  local ampSum = 0.0
...
local PREVIEW_WAVE_TABLE_SIZE = 256

local function lookupPreviewWaveTable(tableData, phase)
  local samples = tableData and tableData.samples
  local sampleCount = tonumber(tableData and tableData.size) or 0
  if type(samples) ~= "table" or sampleCount <= 1 then
    return 0.0
  end

  local wrappedPhase = (tonumber(phase) or 0.0) % 1.0
  local pos = wrappedPhase * sampleCount
  local idx = math.floor(pos)
  local frac = pos - idx
  local i1 = (idx % sampleCount) + 1
  local i2 = ((idx + 1) % sampleCount) + 1
  local s1 = tonumber(samples[i1]) or 0.0
  local s2 = tonumber(samples[i2]) or s1
  return s1 + (s2 - s1) * frac
end

local function getCachedWaveAddPreviewTable(ctx, waveType, pulseWidth)
  local width = math.max(0.01, math.min(0.99, tonumber(pulseWidth) or 0.5))
  local partialCount = math.max(1, math.min(32, math.floor((tonumber(ctx.additivePartials) or 8) + 0.5)))
  local tilt = math.max(-1.0, math.min(1.0, tonumber(ctx.additiveTilt) or 0.0))
  local drift = math.max(0.0, math.min(1.0, tonumber(ctx.additiveDrift) or 0.0))
  local effectiveUnison, effectiveDetune = getWaveAddPreviewVoiceConfig(ctx)
  local key = string.format("%d:%.4f:%d:%.4f:%.4f:%d:%.4f",
    tonumber(waveType) or 0,
    width,
    partialCount,
    tilt,
    drift,
  ...
  if ctx._cachedWaveAddPreviewTableKey ~= key then
    local partials = getCachedAdditiveRecipe(ctx, waveType, width)
    local detuneRatios = getCachedDetuneRatios(ctx, effectiveUnison, effectiveDetune)
    local sampleCount = PREVIEW_WAVE_TABLE_SIZE
    local normVoices = math.max(1, #detuneRatios)
    local tiltComp = 1.0 + math.max(-0.12, math.min(0.10, tilt * -0.10))
    local driftComp = 1.0 - drift * 0.08
    local outputTrim = additiveOutputTrim(waveType) * tiltComp * driftComp
    local samples = {}

    for i = 1, sampleCount do
      local phase = (i - 1) / sampleCount
      local sum = 0.0
      for v = 1, #detuneRatios do
        local detunedPhase = phase * detuneRatios[v]
        if waveType == 7 then
          sum = sum + additiveSuperSawPreview(ctx, detunedPhase, partialCount)
        else
          sum = sum + additiveRecipeSample(partials, detunedPhase)
        end
      end
      samples[i] = math.max(-1.0, math.min(1.0, (sum / math.sqrt(normVoices)) * outputTrim))
    end

    ctx._cachedWaveAddPreviewTableKey = key
    ctx._cachedWaveAddPreviewTable = {
      size = sampleCount,
      samples = samples,
    }
  end

  return ctx._cachedWaveAddPreviewTable
end
...
local function renderWaveSample(ctx, waveType, phase)
  ...
  local previewTable = getCachedWaveAddPreviewTable(ctx, waveType, pulseWidth)
  return lookupPreviewWaveTable(previewTable, phase)
end
...
local function morphPreviewPartials(partialsA, partialsB, position, curve, depth)
  ...
  -- DSP uses effectiveMorphAmount = blend * depth as the morph position.
  -- At depth=0, the DSP produces pure wave; the visual must match.
  local effectivePos = pos * dep
  local aCoeff, bCoeff
  if curve == 0 then aCoeff = 1.0 - effectivePos; bCoeff = effectivePos
  elseif curve == 1 then local t = 0.5 - 0.5 * math.cos(effectivePos * math.pi); aCoeff = 1.0 - t; bCoeff = t
  else aCoeff = math.cos(effectivePos * math.pi * 0.5); bCoeff = math.sin(effectivePos * math.pi * 0.5) end

  local freqT = effectivePos
...
function SourcePanelBehavior.init(ctx)
  ctx.waveformType = 1
  ctx.renderMode = 0
  ctx.pulseWidth = 0.5
  ctx.unison = 1
...
function SourcePanelBehavior.resized(ctx, w, h)
...
function SourcePanelBehavior.repaint(ctx)
  refreshGraph(ctx)
end

SourcePanelBehavior.refreshDriveCurve = refreshDriveCurve
...
function SourcePanelBehavior.updateKnobLayout(ctx)
  local widgets = ctx.widgets
