...
local function moduleComponentId(ctx)
  local props = type(ctx) == 'table' and ctx.instanceProps or nil
  local value = type(props) == 'table' and tostring(props.moduleComponentId or '') or ''
  if value == '' then
...
local function contentReferenceSize(ctx)
  local props = type(ctx) == 'table' and ctx.instanceProps or nil
  local w = math.max(1, math.floor(tonumber(type(props) == 'table' and props.contentW or nil) or DEFAULT_CONTENT_W))
  local h = math.max(1, math.floor(tonumber(type(props) == 'table' and props.contentH or nil) or DEFAULT_CONTENT_H))
  return w, h
end
...
local function layout(ctx)
  ...
  local component = getRuntimeWidget(ctx, moduleComponentId(ctx))
  local settingsOverlay = getRuntimeWidget(ctx, 'settings_overlay')
  local perfOverlay = getRuntimeWidget(ctx, 'perf_overlay')
  local refW, refH = contentReferenceSize(ctx)

  if component and component.node then
    local scale = math.min(contentW / refW, contentH / refH)
    if not scale or scale <= 0 then
      scale = 1
    end
    local moduleW = math.max(1, math.floor(refW * scale + 0.5))
    local moduleH = math.max(1, math.floor(refH * scale + 0.5))
    local moduleX = math.floor((contentW - moduleW) * 0.5 + 0.5)
    local moduleY = HEADER_H + math.floor((contentH - moduleH) * 0.5 + 0.5)
    if component.node.setBounds then
      component.node:setBounds(moduleX, moduleY, moduleW, moduleH)
    end
    setVisible(component, not showSettings)
  end
