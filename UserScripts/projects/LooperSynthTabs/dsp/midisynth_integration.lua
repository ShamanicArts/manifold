-- MidiSynth Integration Module for LooperSynthTabs
...
function M.buildSynth(ctx, options)
  ...
  -- Use a separate send node so `out` remains a graph sink (= goes to host output
  -- for monitoring). The send taps the same signal into the looper capture chain.
