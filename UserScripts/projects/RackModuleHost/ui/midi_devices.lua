...
-- RACKMODULEHOST_SPECIFIC_VERSION

local M = {}

-- Set dropdown options by modifying structured record (for widgets without setOptions method)
local function setDropdownOptions(widget, options)
  if not widget then
  ...
  if widget.setOptions then
    ...
    return true, "used setOptions method"
  end
  -- Fallback: modify structured record directly
  ...
  if record and record.spec and record.spec.props then
    ...
    return true, "set options directly"
  end
  return false, "no _structuredRecord.spec.props"
end

-- Set dropdown selected index by modifying structured record
local function setDropdownSelected(widget, idx)
  if not widget then
    return false
  end
  -- Try standard method first
...
local function setLabelText(widget, text)
  ...
  elseif widget._structuredRecord and widget._structuredRecord.spec and widget._structuredRecord.spec.props then
    widget._structuredRecord.spec.props.text = tostring(text or "")
    if widget.node and widget.node.repaint then
      widget.node:repaint()
    end
  end
...
function M.applyMidiSelection(ctx, idx, persist)
  ...
  -- Support both aliased short names and full paths
  local midiDropdown = widgets.midiInputDropdown or widgets["rack_host_root.sidebar.midi_input_dropdown"]
  local deviceValueLabel = widgets.deviceValue or widgets["rack_host_root.sidebar.midi_device_value"]
  ...
  if idx == 1 then
    ...
    setLabelText(deviceValueLabel, "Input: " .. label)
    if persist then
  ...
  if success then
    ...
    if midiDropdown and midiDropdown.setSelected then
      midiDropdown:setSelected(activeIdx)
    end
    setLabelText(deviceValueLabel, "Input: " .. activeLabel)
    if persist then
...
function M.refreshMidiDevices(ctx, restoreSelection)
  ctx._midiDebug = (ctx._midiDebug or "") .. "|MD.refresh enter"
  local widgets = ctx.widgets or {}
  -- Support both aliased short names and full paths
  local midiDropdown = widgets.midiInputDropdown or widgets["rack_host_root.sidebar.midi_input_dropdown"]
  local deviceValueLabel = widgets.deviceValue or widgets["rack_host_root.sidebar.midi_device_value"]

  ctx._midiDebug = (ctx._midiDebug or "") .. ", md=" .. tostring(midiDropdown ~= nil)

  local options = M.buildMidiOptions(ctx)
  ctx._midiOptions = options
  ctx._lastKnownMidiDeviceCount = math.max(0, #options - 1)

  local ok, err = setDropdownOptions(midiDropdown, options)
  ctx._lastDropdownResult = tostring(ok) .. ":" .. tostring(err)
  ctx._midiDebug = (ctx._midiDebug or "") .. ", result=" .. tostring(ok)
  ...
  setDropdownSelected(midiDropdown, idx)
  ctx._selectedMidiInputIdx = idx
  ...
  if not activeIdx and restoreSelection and idx > 1 then
    ...
    setLabelText(deviceValueLabel, "Input: " .. (ctx._selectedMidiInputLabel or options[1] or "None"))
  end
