...
local function layout(ctx)
  ...
  local filterComponent = getRuntimeWidget(ctx, 'filter_component')
  ...
  if filterComponent and filterComponent.node then
    ...
    if filterComponent.node.setBounds then
      filterComponent.node:setBounds(moduleX, moduleY, moduleW, moduleH)
    end
    setVisible(filterComponent, not showSettings)
  end
