...
local function buildPlugin(ctx)
  local state = {
    super = {
      vocal = nil,
      layerFx = {},
  ...
  local function createFxSlot(basePath)
    ...
    for idx, def in ipairs(effectDefs) do
      ...
      for _, param in ipairs(def.params) do
        ...
        param.apply(effect, param.default)
        slot.paramBindings[path] = function(value)
          param.apply(effect, value)
        end
      end
    end
  ...
  for i = 0, 3 do
    ...
    if layerOut then
      ...
      ctx.graph.connect(slot.output, layerMixer, 0, i)
    end

    if layerIn then
      ctx.graph.connect(state.super.vocal.output, layerIn)
    end
  ...
  ctx.graph.connect(state.super.vocal.output, mainMixer, 0, 1)
  ctx.graph.connect(mainMixer, masterGain)

  local function applySlotParam(path, value)
    if state.super.vocal.applyParam(path, value) then
      return true
    end
    for i = 1, #state.super.layerFx do
      if state.super.layerFx[i].applyParam(path, value) then
        return true
      end
    end
    return false
