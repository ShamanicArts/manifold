...
function M.refreshMidiDevices(ctx)
  ...
  if Midi.currentInputDeviceIndex and Midi.isInputOpen and Midi.isInputOpen() then
    selectedInput = (Midi.currentInputDeviceIndex() or -1) + 2
  end

  local selectedOutput = 1
  if Midi.currentOutputDeviceIndex and Midi.isOutputOpen and Midi.isOutputOpen() then
    selectedOutput = (Midi.currentOutputDeviceIndex() or -1) + 2
  end
  
  if w.midiInputDropdown then
    w.midiInputDropdown:setOptions(inputOptions)
    w.midiInputDropdown:setSelected(selectedInput)
  end
  if w.midiOutputDropdown then
    w.midiOutputDropdown:setOptions(outputOptions)
    w.midiOutputDropdown:setSelected(selectedOutput)
  end
end
...
-- Overlay-safe monitor: read latest MIDI message without consuming Main's ring
function M.updateMidiMonitorPeek(ctx)
  if not Midi or not Midi.peekLatestInputEvent then return end
  if M.currentTab ~= "midi" then return end

  local w = ctx.widgets
  if not w.midiEventDisplay then return end

  local event = Midi.peekLatestInputEvent()
  if not event then
    if w.midiActivityLed and w.midiActivityLed.setStyle then
      w.midiActivityLed:setStyle({ bg = 0xff333333 })
    end
  ...
  if ctx._lastMidiMonitorSeq == event.seq then
    return
  end
  ctx._lastMidiMonitorSeq = event.seq
...
function M.init(ctx)
  ...
  -- Wire close button to dismiss the overlay
  if w.close_btn then
    w.close_btn._onClick = function()
      if type(closeOverlay) == "function" then
        closeOverlay()
      end
    end
  end
...
function M.update(ctx, state)
  ...
  -- In overlay mode, use non-destructive MIDI monitor tap.
  if isOverlayActive and isOverlayActive() then
    M.updateMidiMonitorPeek(ctx)
    return
  end

  -- Normal mode: consume ring directly
