...
struct ParseResult {
  enum class Kind {
    ...
    RecordStart,     // RECORD START [duration] [format] [path] - start recording
    RecordStop,      // RECORD STOP - stop recording
    RecordStatus,    // RECORD_STATUS - query recording state
  ...
  std::string capturePath;  // valid when kind == Screenshot / RecordStart
  std::string recordFormat; // valid when kind == RecordStart (png, jpg, webm)
  int recordDuration = 0;  // valid when kind == RecordStart (seconds, 0=manual stop)
...
inline ParseResult parse(const std::string &cmd,
  ...
  if (verb == "SCREENSHOT") {
    ParseResult r;
    r.kind = ParseResult::Kind::Screenshot;
    // Default path will be generated by ControlServer if not provided
    if (tokens.size() >= 2) {
      // Join remaining tokens as path (may contain spaces)
      r.capturePath = tokens[1];
      for (size_t i = 2; i < tokens.size(); ++i) {
        r.capturePath += " ";
        r.capturePath += tokens[i];
      }
    }
    return r;
  }

  // ---- RECORD START [duration] [format] [path] ----
  if (verb == "RECORD" && tokens.size() >= 2 && toUpper(tokens[1]) == "START") {
    ParseResult r;
    r.kind = ParseResult::Kind::RecordStart;
    r.recordFormat = "png";  // default format
    r.recordDuration = 0;     // default: manual stop

    // Parse remaining: [duration] [format] [path]
    ...
    if (idx < tokens.size()) {
      ...
      int duration = 0;
      if (parseIntToken(tokens[idx], duration) && duration > 0) {
        r.recordDuration = duration;
        ++idx;
      }
    }
    if (idx < tokens.size()) {
      const auto upperFormat = toUpper(tokens[idx]);
      if (upperFormat == "PNG" || upperFormat == "JPG" || upperFormat == "JPEG" || upperFormat == "WEBM" || upperFormat == "TGA") {
        r.recordFormat = toLower(tokens[idx]);
        ++idx;
      }
    }
    if (idx < tokens.size()) {
      r.capturePath = tokens[idx];
  ...
  // ---- RECORD STOP ----
  if (verb == "RECORD" && tokens.size() >= 2 && toUpper(tokens[1]) == "STOP") {
    ParseResult r;
    r.kind = ParseResult::Kind::RecordStop;
    return r;
  }

  // ---- RECORD STATUS ----
  if (verb == "RECORD" && tokens.size() >= 2 && toUpper(tokens[1]) == "STATUS") {
    return makeQuery("RECORD_STATUS");
  }
